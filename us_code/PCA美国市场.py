...
pca.fit(x[:'1988'].loc[:,size])
size = pca.transform(x.loc[:, size])

pca = PCA(n_components=1)
pca.fit(x[:'1988'].loc[:,beta])
beta = pca.transform(x.loc[:, beta])

pca = PCA(n_components=1)
pca.fit(x[:'1988'].loc[:,mom])
mom = pca.transform(x.loc[:, mom])

pca = PCA(n_components=1)
pca.fit(x[:'1988'].loc[:,liq])
liq = pca.transform(x.loc[:, liq])

pca = PCA(n_components=1)
pca.fit(x[:'1988'].loc[:,vol])
vol = pca.transform(x.loc[:, vol])

pca = PCA(n_components=1)
pca.fit(x[:'1988'].loc[:,bpr])
bpr = pca.transform(x.loc[:, bpr])

pca = PCA(n_components=1)
pca.fit(x[:'1988'].loc[:,ey])
ey = pca.transform(x.loc[:, ey])

pca = PCA(n_components=1)
pca.fit(x[:'1988'].loc[:,growth])
growth = pca.transform(x.loc[:, growth])

pca = PCA(n_components=1)
pca.fit(x[:'1988'].loc[:,lever])
lever = pca.transform(x.loc[:, lever])
#%%
col = ['size', 'beta', 'mom', 'liq', 'vol', 'bpr', 'ey', 'growth', 'lever']+['sin', 'baspread']
pca_data = np.hstack([size, beta, mom, liq, vol, bpr, ey, growth, lever, x[['sin', 'baspread']].values])
pca_data = pd.DataFrame(pca_data).fillna(0)
pca_data.index=x.index
pca_data.columns=col
pca_data
...
x_array = [pca_data]
for i in range(8):
    x_array.append(pca_data * np.array(macro_data.iloc[:, i]).reshape(-1, 1))
...
#                         verbose=0)
#     model = GridSearchCV(ENet, param_grid, cv=ps, n_jobs=5)
#
...
#
#     # pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/美国市场预测结果/PCA/Ridge-{}.csv'.format(year + 1), index=False)
#     #
#     # # 因子重要性
...
#     #         x2.iloc[:, i + (j * 11)] = 0
#     #     pd.DataFrame(
#     #         model.predict(x2)).to_csv('/20230328代码与数据/美国市场预测结果/PCA/Ridge-{}-{}.csv'.format(year+1,feature),index=False)
#
#     loss.append(history)
...
# R方_RF = 1 - (sum((np.array(y_test).reshape(len(y_test), 1) - np.array(data['pred']).reshape(-1, 1)) ** 2)) / (
#     sum((np.array(y_test).reshape(len(y_test), 1) ** 2)))
...
#     print('ENet的R方：', R方_RF)
#
#     # pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/美国市场预测结果/PCA/ENet-{}.csv'.format(year + 1), index=False)
#     #
#     # # 因子重要性
#     # col = ['size', 'beta', 'mom', 'liq', 'vol', 'bpr', 'ey', 'growth', 'lever']
#     # for i in range(9):
...
#     #         x2.iloc[:, i + (j * 11)] = 0
#     #     pd.DataFrame(
#     #         model.predict(x2)).to_csv('/20230328代码与数据/美国市场预测结果/PCA/ENet-{}-{}.csv'.format(year+1,feature),index=False)
#
#     loss.append(history)
#     Y_test.append(y_test)
#     Y_pre.append(pred_y3)
...
#     print('OLS的R方：', R方_RF)
#
#     # pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/美国市场预测结果/PCA/OLS-{}.csv'.format(year + 1), index=False)
#     #
#     # # 因子重要性
#     # col = ['size', 'beta', 'mom', 'liq', 'vol', 'bpr', 'ey', 'growth', 'lever']
#     # for i in range(9):
...
#     #         x2.iloc[:, i + (j * 11)] = 0
#     #     pd.DataFrame(
#     #         model.predict(x2)).to_csv('/20230328代码与数据/美国市场预测结果/PCA/OLS-{}-{}.csv'.format(year+1,feature),index=False)
#
#     loss.append(history)
#     Y_test.append(y_test)
#     Y_pre.append(pred_y3)
...
#
#     # pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/美国市场预测结果/PCA/Lasso-{}.csv'.format(year + 1), index=False)
#     #
#     # # 因子重要性
#     # col = ['size', 'beta', 'mom', 'liq', 'vol', 'bpr', 'ey', 'growth', 'lever']
#     # for i in range(9):
...
#     #         x2.iloc[:, i + (j * 11)] = 0
#     #     pd.DataFrame(
#     #         model.predict(x2)).to_csv('/20230328代码与数据/美国市场预测结果/PCA/Lasso-{}-{}.csv'.format(year+1,feature),index=False)
#
#     loss.append(history)
#     Y_test.append(y_test)
#     Y_pre.append(pred_y3)
...
for year in range(19):
    ...
    model = RandomForestRegressor(n_jobs=20)  # random_state=1,
    model = RandomizedSearchCV(model, param_grid, cv=ps, n_jobs=20, n_iter=15
                         )
    history = model.fit(x_train_val, y_train_val)
    ...
    # pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/美国市场预测结果/PCA/RF-{}.csv'.format(year + 1), index=False)
    #
    # # 因子重要性
    # col = ['size', 'beta', 'mom', 'liq', 'vol', 'bpr', 'ey', 'growth', 'lever']
    # for i in range(9):
    ...
    #         x2.iloc[:, i + (j * 11)] = 0
    #     pd.DataFrame(
    #         model.predict(x2)).to_csv('/20230328代码与数据/美国市场预测结果/PCA/RF-{}-{}.csv'.format(year+1,feature),index=False)
...
for year in range(19):
    ...
    # pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/美国市场预测结果/PCA/NN3-{}.csv'.format(year + 1), index=False)
    #
    # # 因子重要性
    # col = ['size', 'beta', 'mom', 'liq', 'vol', 'bpr', 'ey', 'growth', 'lever']
    # for i in range(9):
    ...
    #     pd.DataFrame(
    #         model.predict(x2)).to_csv('/20230328代码与数据/美国市场预测结果/PCA/NN3-{}-{}.csv'.format(year+1,feature),index=False)
