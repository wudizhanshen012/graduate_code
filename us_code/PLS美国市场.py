...
df.iloc[:, 1:] = df.iloc[:, 1:] - df.iloc[:, 1:].shift(1)
...
pls.fit(x[:'1988'].loc[:,size], y[:'1988'])
size = pls.transform(x.loc[:, size])

pls = PLSRegression(n_components=1)
pls.fit(x[:'1988'].loc[:,beta], y[:'1988'])
beta = pls.transform(x.loc[:, beta])

pls = PLSRegression(n_components=1)
pls.fit(x[:'1988'].loc[:,mom], y[:'1988'])
mom = pls.transform(x.loc[:, mom])

pls = PLSRegression(n_components=1)
pls.fit(x[:'1988'].loc[:,liq], y[:'1988'])
liq = pls.transform(x.loc[:, liq])

pls = PLSRegression(n_components=1)
pls.fit(x[:'1988'].loc[:,vol], y[:'1988'])
vol = pls.transform(x.loc[:, vol])

pls = PLSRegression(n_components=1)
pls.fit(x[:'1988'].loc[:,bpr], y[:'1988'])
bpr = pls.transform(x.loc[:, bpr])

pls = PLSRegression(n_components=1)
pls.fit(x[:'1988'].loc[:,ey], y[:'1988'])
ey = pls.transform(x.loc[:, ey])

pls = PLSRegression(n_components=1)
pls.fit(x[:'1988'].loc[:,growth], y[:'1988'])
growth = pls.transform(x.loc[:, growth])

pls = PLSRegression(n_components=1)
pls.fit(x[:'1988'].loc[:,lever], y[:'1988'])
lever = pls.transform(x.loc[:, lever])
#%%
col = ['size', 'beta', 'mom', 'liq', 'vol', 'bpr', 'ey', 'growth', 'lever']+['sin', 'baspread']
pls_data = np.hstack([size, beta, mom, liq, vol, bpr, ey, growth, lever, x[['sin', 'baspread']].values])
...
for year in range(19):
    ...
    # pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/美国市场预测结果/PLS/Ridge-{}.csv'.format(year + 1), index=False)
    #
    # # 因子重要性
    ...
    #         x2.iloc[:, i + (j * 11)] = 0
    #     pd.DataFrame(
    #         model.predict(x2)).to_csv('/20230328代码与数据/美国市场预测结果/PLS/Ridge-{}-{}.csv'.format(year + 1, feature),
    #                                   index=False)
...
#     print('ENet的R方：', R方_RF)
#
#     pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/美国市场预测结果/PLS/ENet-{}.csv'.format(year + 1), index=False)
#
#     # 因子重要性
#     col = ['size', 'beta', 'mom', 'liq', 'vol', 'bpr', 'ey', 'growth', 'lever']
#     for i in range(9):
...
#             x2.iloc[:, i + (j * 11)] = 0
#         pd.DataFrame(
#             model.predict(x2)).to_csv('/20230328代码与数据/美国市场预测结果/PLS/ENet-{}-{}.csv'.format(year + 1, feature),
#                                       index=False)
#
#     loss.append(history)
#     Y_test.append(y_test)
...
#     print('OLS的R方：', R方_RF)
#
#     pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/美国市场预测结果/PLS/OLS-{}.csv'.format(year + 1), index=False)
#
#     # 因子重要性
#     col = ['size', 'beta', 'mom', 'liq', 'vol', 'bpr', 'ey', 'growth', 'lever']
#     for i in range(9):
...
#             x2.iloc[:, i + (j * 11)] = 0
#         pd.DataFrame(
#             model.predict(x2)).to_csv('/20230328代码与数据/美国市场预测结果/PLS/OLS-{}-{}.csv'.format(year + 1, feature),
#                                       index=False)
#
#     loss.append(history)
#     Y_test.append(y_test)
...
#
#     pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/美国市场预测结果/PLS/Lasso-{}.csv'.format(year + 1), index=False)
#
#     # 因子重要性
#     col = ['size', 'beta', 'mom', 'liq', 'vol', 'bpr', 'ey', 'growth', 'lever']
#     for i in range(9):
...
#             x2.iloc[:, i + (j * 11)] = 0
#         pd.DataFrame(
#             model.predict(x2)).to_csv('/20230328代码与数据/美国市场预测结果/PLS/Lasso-{}-{}.csv'.format(year + 1, feature),
#                                       index=False)
#
#     loss.append(history)
#     Y_test.append(y_test)
...
for year in range(19):
    ...
    test_fold[:x_train.shape[0]] = -1  # 将训练集对应的index设为-1，表示永远不划分到验证集中
    ps = PredefinedSplit(test_fold=test_fold)
    param_grid = {'max_features': [5, 10, 20, 40],
                  'max_depth': [1, 2, 3]}
    model = RandomForestRegressor(n_estimators=300, random_state=0, n_jobs=15)
    model = GridSearchCV(model, param_grid, cv=ps
                         )
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/美国市场预测结果/PLS/RF-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        for j in range(8):
            x2.iloc[:, i + (j * 11)] = 0
        pd.DataFrame(
            model.predict(x2)).to_csv('/20230328代码与数据/美国市场预测结果/PLS/RF-{}-{}.csv'.format(year+1,feature),index=False)
...
for year in range(19):
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/美国市场预测结果/PLS/NN3-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        pd.DataFrame(
            model.predict(x2)).to_csv('/20230328代码与数据/美国市场预测结果/PLS/NN3-{}-{}.csv'.format(year+1,feature),index=False)
