...
# 对data按照ym列进行分组，并对每组中的x_col列表中的元素进行标准化，给出代码
data[x_col] = data.groupby('ym').apply(lambda x: 2 * (x[x_col] - x[x_col].min())/(x[x_col].max() - x[x_col].min()) - 1)
data[y_col[0]] -= data['Rf+1']

# 中值填充x
data = data.replace([np.inf, -np.inf], np.nan)
for x in x_col:
    data[x].fillna(value=data.groupby('ym')[x].transform('median'),
                                inplace=True)
