...
pls.fit(x_fit.loc[:,size],y_fit['ret+1'])
size = pls.transform(x.loc[:,size],y['ret+1'])[0]

pls = PLSRegression(n_components=1)
pls.fit(x_fit.loc[:,beta],y_fit['ret+1'])
beta = pls.transform(x.loc[:,beta],y['ret+1'])[0]

pls = PLSRegression(n_components=1)
pls.fit(x_fit.loc[:,mom],y_fit['ret+1'])
mom = pls.transform(x.loc[:,mom],y['ret+1'])[0]

pls = PLSRegression(n_components=1)
pls.fit(x_fit.loc[:,liq],y_fit['ret+1'])
liq = pls.transform(x.loc[:,liq],y['ret+1'])[0]

pls = PLSRegression(n_components=1)
pls.fit(x_fit.loc[:,vol],y_fit['ret+1'])
vol = pls.transform(x.loc[:,vol],y['ret+1'])[0]

pls = PLSRegression(n_components=1)
pls.fit(x_fit.loc[:,bpr],y_fit['ret+1'])
bpr = pls.transform(x.loc[:,bpr],y['ret+1'])[0]

pls = PLSRegression(n_components=1)
pls.fit(x_fit.loc[:,ey],y_fit['ret+1'])
ey = pls.transform(x.loc[:,ey],y['ret+1'])[0]

pls = PLSRegression(n_components=1)
pls.fit(x_fit.loc[:,growth],y_fit['ret+1'])
growth = pls.transform(x.loc[:,growth],y['ret+1'])[0]

pls = PLSRegression(n_components=1)
pls.fit(x_fit.loc[:,lever],y_fit['ret+1'])
lever = pls.transform(x.loc[:,lever],y['ret+1'])[0]
#%%
col = ['size', 'beta', 'mom', 'liq', 'vol', 'bpr', 'ey', 'growth', 'lever']+['largestholderrate', 'top10holderrate', 'soe', 'private', 'foreign', 'others']
pls_data = np.hstack([size, beta, mom, liq, vol, bpr, ey, growth, lever, x[['largestholderrate', 'top10holderrate', 'soe', 'private', 'foreign', 'others']].values])
pls_data = pd.DataFrame(pls_data).fillna(0)
pls_data.index=x.index
pls_data.columns=col
pls_data
...
x_array = [pls_data]
for i in range(11):
    x_array.append(pls_data * np.array(macro_data.iloc[:, i]).reshape(-1, 1))
...
for year in range(10):
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/中国市场预测结果/PLS/OLS-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        for j in range(11):
            x2.iloc[:, i + (j * 15)] = 0
        pd.DataFrame(
            model.predict(x2)).to_csv('/20230328代码与数据/中国市场预测结果/PLS/OLS-{}-{}.csv'.format(year + 1, feature),
                                      index=False)
...
for year in range(10):
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/中国市场预测结果/PLS/Lasso-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        for j in range(11):
            x2.iloc[:, i + (j * 15)] = 0
        pd.DataFrame(
            model.predict(x2)).to_csv('/20230328代码与数据/中国市场预测结果/PLS/Lasso-{}-{}.csv'.format(year + 1, feature),
                                      index=False)
...
for year in range(10):
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/中国市场预测结果/PLS/Ridge-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        for j in range(11):
            x2.iloc[:, i + (j * 15)] = 0
        pd.DataFrame(
            model.predict(x2)).to_csv('/20230328代码与数据/中国市场预测结果/PLS/Ridge-{}-{}.csv'.format(year + 1, feature),
                                      index=False)
...
for year in range(10):
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/中国市场预测结果/PLS/ENet-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        for j in range(11):
            x2.iloc[:, i + (j * 15)] = 0
        pd.DataFrame(
            model.predict(x2)).to_csv('/20230328代码与数据/中国市场预测结果/PLS/ENet-{}-{}.csv'.format(year + 1, feature),
                                      index=False)
...
for year in range(10):
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/中国市场预测结果/PLS/RF-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        for j in range(11):
            x2.iloc[:, i + (j * 15)] = 0
        pd.DataFrame(
            model.predict(x2)).to_csv('/20230328代码与数据/中国市场预测结果/PLS/RF-{}-{}.csv'.format(year + 1, feature),
                                      index=False)
...
for year in range(10):
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/中国市场预测结果/PLS/NN3-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        pd.DataFrame(pred).to_csv('/20230328代码与数据/中国市场预测结果/PLS/NN3-{}-{}.csv'.format(year + 1, feature), index=False)
