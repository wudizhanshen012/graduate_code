...
pca.fit(x[x['y']<=2008].loc[:,size])
size = pca.transform(x.loc[:, size])
pca = PCA(n_components=1)
pca.fit(x[x['y']<=2008].loc[:,beta])
beta = pca.transform(x.loc[:, beta])
pca = PCA(n_components=1)
pca.fit(x[x['y']<=2008].loc[:,mom])
mom = pca.transform(x.loc[:, mom])
pca = PCA(n_components=1)
pca.fit(x[x['y']<=2008].loc[:,liq])
liq = pca.transform(x.loc[:, liq])
pca = PCA(n_components=1)
pca.fit(x[x['y']<=2008].loc[:,vol])
vol = pca.transform(x.loc[:, vol])
pca.fit(x[x['y']<=2008].loc[:,bpr])
bpr = pca.transform(x.loc[:, bpr])
pca.fit(x[x['y']<=2008].loc[:,ey])
ey = pca.transform(x.loc[:, ey])
pca.fit(x[x['y']<=2008].loc[:,growth])
growth = pca.transform(x.loc[:, growth])
pca.fit(x[x['y']<=2008].loc[:,lever])
...
pca_data = np.hstack([size, beta, mom, liq, vol, bpr, ey, growth, lever, x[['largestholderrate', 'top10holderrate', 'soe', 'private', 'foreign', 'others']].values])
...
for year in range(10):
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/中国市场预测结果/PCA/OLS-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        for j in range(11):
            x2.iloc[:, i + (j * 15)] = 0
        pd.DataFrame(
            model.predict(x2)).to_csv('/20230328代码与数据/中国市场预测结果/PCA/OLS-{}-{}.csv'.format(year + 1, feature),
                                      index=False)
...
for year in range(10):
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/中国市场预测结果/PCA/Lasso-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        for j in range(11):
            x2.iloc[:, i + (j * 15)] = 0
        pd.DataFrame(
            model.predict(x2)).to_csv('/20230328代码与数据/中国市场预测结果/PCA/Lasso-{}-{}.csv'.format(year + 1, feature),
                                      index=False)
...
for year in range(10):
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/中国市场预测结果/PCA/Ridge-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        for j in range(11):
            x2.iloc[:, i + (j * 15)] = 0
        pd.DataFrame(
            model.predict(x2)).to_csv('/20230328代码与数据/中国市场预测结果/PCA/Ridge-{}-{}.csv'.format(year + 1, feature),
                                      index=False)
...
for year in range(10):
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/中国市场预测结果/PCA/ENet-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        for j in range(11):
            x2.iloc[:, i + (j * 15)] = 0
        pd.DataFrame(
            model.predict(x2)).to_csv('/20230328代码与数据/中国市场预测结果/PCA/ENet-{}-{}.csv'.format(year + 1, feature),
                                      index=False)
...
for year in range(10):
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/中国市场预测结果/PCA/RF-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        for j in range(11):
            x2.iloc[:, i + (j * 15)] = 0
        pd.DataFrame(
            model.predict(x2)).to_csv('/20230328代码与数据/中国市场预测结果/PCA/RF-{}-{}.csv'.format(year + 1, feature),
                                      index=False)
...
for year in range(10):
    ...
    pd.DataFrame(pred_y3).to_csv('/20230328代码与数据/中国市场预测结果/PCA/NN3-{}.csv'.format(year + 1), index=False)
    ...
    for i in range(9):
        ...
        pd.DataFrame(pred).to_csv('/20230328代码与数据/中国市场预测结果/PCA/NN3-{}-{}.csv'.format(year + 1, feature), index=False)
